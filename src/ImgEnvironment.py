...
class Environment(object):
    ...
    def __init__(self, strengthA, strengthB):
        ...
        self.resolution = 80  # size of the image in pixel
        self.resolutionOversize = 20  # black pixels between the target border and the image border
    ...
    def __calcImage(self, elegantOutput):
        ...
        xCoordinate *= (self.resolution - self.resolutionOversize) / (2 * self.targetRadius)
        yCoordinate *= (self.resolution - self.resolutionOversize) / (2 * self.targetRadius)

        image = torch.zeros(self.resolution, self.resolution)

        countSkipped = 0

        # iterate over particles
        for particle in range(len(xCoordinate)):
            if torch.sqrt(xCoordinate[particle] ** 2 + yCoordinate[particle] ** 2) > (self.resolution - self.resolutionOversize) / 2:
                # particle not on target
                countSkipped += 1

                continue

            xPixel = int(xCoordinate[particle]) + self.resolution // 2
            yPixel = int(yCoordinate[particle]) + self.resolution // 2
...
if __name__ == '__main__':
    ...
    action = Action(state, torch.tensor((0.04, -0.04)))
    ...
    scaling = (env.resolution + env.resolutionOversize) / env.resolution
    notation = np.linspace(-env.targetRadius * scaling, env.targetRadius * scaling, env.resolution, endpoint=True)
